...
RSpec.describe RawgSearchService do
  describe '#rawg_service' do
    it 'returns a specific games and its details', :vcr do
      ...
      expect(response).to have_key(:platforms)
      expect(response[:platforms]).to be_a(Array)
      expect(response[:platforms][0]).to have_key(:platform)
      expect(response[:platforms][0][:platform]).to be_a(Hash)
      expect(response[:platforms][0][:platform]).to have_key(:name)
      expect(response[:platforms][0][:platform][:name]).to be_a(String)
